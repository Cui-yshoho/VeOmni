...
class ExpertToyExperts(nn.Module):
    def __init__(self):
        super().__init__()
        # Keep every expert distinguishable so checkpoint tests detect an EP
        # shard being mistaken for a replica of another rank's shard.
        self.weight = nn.Parameter(torch.arange(8 * 16 * 32, dtype=torch.float32).reshape(8, 16, 32) / 4096)
...
def _run_checkpoint(args, world_size):
    _init_parallel_state(dp_size=world_size, dp_mode="fsdp2", device_type="npu")
...
def _run_extra_parallel(args, world_size):
    ...
    if args.checkpoint_path:
        ...
        checkpoint_model_state = _full_state(checkpoint_model)
        ...
        resume_state = {"model": resumed_model, "optimizer": resumed_optimizer, "extra_state": {}}
        HyperParallelCheckpointer.load(args.checkpoint_path, resume_state)
        _assert_state_equal(_full_state(resumed_model), checkpoint_model_state, "model")
