...
def _mesh_signature(mesh) -> Optional[Dict[str, Any]]:
    if mesh is None:
        return None
    return {
        "dim_names": list(mesh.mesh_dim_names or ()),
        "ranks": mesh.mesh.cpu().tolist(),
    }


def _topology_signature(model, parallel_state) -> Dict[str, Any]:
    ...
    specs = []
    for fqn, info in sorted(getattr(model, "_fqn2spec_info", {}).items()):
        placement = info.placement
        specs.append(
            {
                "fqn": fqn,
                "parallel": info.para_name,
                "placement": type(placement).__name__,
                "placement_dim": getattr(placement, "dim", None),
                "fsdp_shard_dim": info.fsdp_shard_dim,
            }
        )
    ...
    return {
        ...
        "parallel_sizes": {
            "dp_replicate": parallel_state.dp_replicate_size,
            "dp_shard": parallel_state.dp_shard_size,
            "tp": parallel_state.tp_size,
            "pp": parallel_state.pp_size,
            "cp": parallel_state.cp_size,
            "ulysses": parallel_state.ulysses_size,
            "extra": dict(sorted(parallel_state.extra_parallel_sizes.items())),
        },
        "device_mesh": _mesh_signature(parallel_state.device_mesh),
        "extra_parallel_meshes": {
            name: _mesh_signature(parallel_state.extra_parallel_fsdp_device_mesh[name])
            for name in sorted(parallel_state.extra_parallel_names)
        },
        "extra_parallel_specs": specs,
    }


def _validate_topology(marker_path: str, model, parallel_state) -> None:
    try:
        with open(marker_path, encoding="utf-8") as marker:
            saved = json.load(marker)
    except (OSError, json.JSONDecodeError) as exc:
        raise RuntimeError(f"Invalid HyperParallel checkpoint completion marker: {marker_path}") from exc

    current = _topology_signature(model, parallel_state)
    if saved != current:
        raise RuntimeError(
            "Rank-local HyperParallel checkpoints require the same world size, global-rank mapping, "
            "parallel topology, and ExtraParallel plan used when saving."
        )
...
class HyperParallelCheckpointer(DistributedCheckpointer):
    ...
    def save(
        ...
        if stage_dir is not None:
            raise NotImplementedError("Checkpoint staging is not supported by Hyper FSDP2 yet.")
        ...
        hp_save({"model": model.state_dict()}, checkpoint_id=f"{checkpoint_dir}/model", use_collectives=False)
        ...
        if not dist.is_initialized() or dist.get_rank() == 0:
            ...
            with open(temporary_marker, "w", encoding="utf-8") as marker:
                json.dump(_topology_signature(model, parallel_state), marker, sort_keys=True)
                marker.write("\n")
    ...
    def load(
        ...
        model = state["model"]
        _validate_topology(os.path.join(path, _HYPER_COMPLETION_MARKER), model, parallel_state)
        model_state = model.state_dict()
        hp_load({"model": model_state}, checkpoint_id=f"{path}/model", use_collectives=False)
